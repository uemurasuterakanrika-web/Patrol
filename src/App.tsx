...
export default function App() {
 ...
 // Only re-setup if active IDs change
  ...
  const handleDeleteInspection = async (e: React.MouseEvent, inspectionId: string) => {
    if (!confirm("この点検記録をゴミ箱に移動しますか？\n（30日間は復元できます）")) return;
    try {
      await api.softDeleteInspection(inspectionId);
  ...
  const loadDeletedInspections = async () => {
    try {
      const deleted = await api.getDeletedInspections();
      setDeletedInspections(deleted);
    } catch (err) {
      console.error("Load deleted inspections error:", err);
    }
  };

  const handleRestoreInspection = async (inspectionId: string) => {
    setIsRestoringId(inspectionId);
    try {
      await api.restoreInspection(inspectionId);
      // ゴミ箱リストから除去して通常リストに戻す
      setDeletedInspections(prev => prev.filter(i => i.id !== inspectionId));
      alert("点検記録を復元しました。");
    } catch (err) {
      console.error("Restore inspection error:", err);
      alert("復元に失敗しました。");
    } finally {
  ...
  const handlePermanentDeleteInspection = async (inspectionId: string) => {
    if (!confirm("この点検記録を完全に削除しますか？\n（この操作は取り消せません）")) return;
    try {
      ...
      <div className="flex-1 flex overflow-hidden no-print">
        ...
        {showTrash && (
          <>
            ...
            />
            <div className="fixed inset-x-4 top-16 bottom-16 max-w-lg mx-auto bg-white rounded-2xl shadow-2xl z-50 flex flex-col overflow-hidden">
              {/* ヘッダー */}
              <div className="p-4 border-b border-stone-100 flex items-center justify-between bg-rose-50">
                <div className="flex items-center gap-2">
                  <Trash2 className="w-5 h-5 text-rose-500" />
                  <h2 className="font-bold text-stone-800">ゴミ箱</h2>
                  <span className="text-xs text-stone-500 ml-1">（30日後に自動削除）</span>
                </div>
                <button
                  onClick={() => setShowTrash(false)}
                  className="p-1.5 hover:bg-rose-100 rounded-lg text-stone-500"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-2">
                {deletedInspections.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-stone-400 gap-3 py-12">
                    <Trash2 className="w-10 h-10 opacity-30" />
                    <p className="text-sm">ゴミ箱は空です</p>
                  </div>
                ) : (
                  deletedInspections.map(insp => {
                    const deletedDaysAgo = insp.deletedAt
                      ? Math.floor((Date.now() - insp.deletedAt) / (1000 * 60 * 60 * 24))
                      : 0;
                    const remainingDays = 30 - deletedDaysAgo;
                    return (
                      <div
                        key={insp.id}
                        className="flex items-start gap-3 p-3 bg-stone-50 border border-stone-100 rounded-xl"
                      >
                        <div className="flex-1 min-w-0">
                          <div className="font-medium text-sm text-stone-800 line-clamp-1">
                            {insp.siteName || '名称未設定'}
                          </div>
                          <div className="text-xs text-stone-500 mt-0.5">
                            {insp.label || insp.date}
                          </div>
                          <div className={`text-xs mt-1 font-semibold ${remainingDays <= 7 ? 'text-rose-500' : 'text-amber-500'}`}>
                            あと {remainingDays} 日で完全削除
                          </div>
                        </div>
                        <div className="flex flex-col gap-1.5 shrink-0">
                          <button
                            onClick={() => handleRestoreInspection(insp.id)}
                            disabled={isRestoringId === insp.id}
                            className="flex items-center gap-1 px-2.5 py-1.5 bg-emerald-500 text-white text-xs font-bold rounded-lg hover:bg-emerald-600 disabled:opacity-50"
                          >
                            {isRestoringId === insp.id ? (
                          ...
                          <button
                            onClick={() => handlePermanentDeleteInspection(insp.id)}
                            className="flex items-center gap-1 px-2.5 py-1.5 bg-stone-100 text-rose-600 text-xs font-bold rounded-lg hover:bg-rose-50"
                          >
